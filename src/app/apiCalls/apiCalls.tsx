...
export async function searchNews(keyword: string) {
  const response = await fetch(`https://d3b4357e-5e24-418e-a017-4d0f0220a96d.mock.pstmn.io/fetchBitcoin`)
...
export async function fetchTopNews() {
  const response = await fetch('https://d3b4357e-5e24-418e-a017-4d0f0220a96d.mock.pstmn.io/fetchTopNews')
...
export async function fetchNewsByDate(date:string, source: string) {
  const response = await fetch(`https://newsapi.org/v2/everything?sources=${source}&from=${subtractSecond(date)}&to=${addSecond(date)}&apiKey=d08187d6de4149389c0d5afb4aa2f40e`)
  console.log('date', date)
  console.log('subtract', subtractSecond(date))
  console.log('add', addSecond(date))
  const data:ReturnData = await response.json()
  return data
}